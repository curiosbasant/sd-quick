...
export async function completeStudentGeneralProfile(
  ...
  if (!result.status && result.error.type === 'error_duplicate_student_mobile_maximum_use') {
    console.log('Retrying caught error, error_duplicate_student_mobile_maximum_use', result.error)
    await sleep(3000)
    return completeStudentGeneralProfile(
      profile,
      payload,
      overrides ?
        { primaryMobile: profile.primaryMobile, secondaryMobile: profile.secondaryMobile }
      : { secondaryMobile: '' },
    )
  }
